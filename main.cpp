...
int main(int argc, char* argv[])
{
    ...
    O,o  Release next cube from its fixed position (1st press = bottom cube, 2nd = middle, 3rd = top)
    ...
    // NEW: State to track which cubes are fixed/released 
    // Based on jelly2.json, we have 3 cubes (8 vertices each): 0 (bottom), 1 (middle), 2 (top)
    // true = fixed (held up), false = released (falling)
    std::vector<bool> cube_is_fixed = { true, true, true };
    ...
    const auto step = [&]()
        {
            ...
            // Apply gravity only to released cubes
            const int verts_per_cube = 8;
            for (size_t i = 0; i < cube_is_fixed.size(); ++i) {
                if (!cube_is_fixed[i]) {
                    int start_index = i * verts_per_cube;
                    for (int j = 0; j < verts_per_cube; ++j) {
                        fext(start_index + j, 1) = -9.8;
                    }
                }
            }
    ...
    viewer.callback_key_pressed = [&](
        ...
        )->bool
        {
            switch (key)
            {
                // NEW: Logic for 'O'/'o' key press
            case 'O':
            case 'o':
                for (size_t i = 0; i < cube_is_fixed.size(); ++i) {
                    if (cube_is_fixed[i] == true) {
                        cube_is_fixed[i] = false; // Release this cube
            ...
            case 'r':
                ...
                // Reset the cube using R
                cube_is_fixed = { true, true, true };
                update();
                return true;
            }
        };
